// script.js -- logique du quiz Jàng Afrig

// --- Générer une question avec 4 propositions (1 bonne + 3 distracteurs) ---
function genererQuestion() {
    const bonPays = choisirAuHasard(pays);       // on tire un pays au hasard
    const bonneReponse = bonPays.capitale;        // sa capitale = bonne réponse
    ...
    while (propositions.length < 4) {
        ...
        if (!propositions.includes(autre.capitale)) { // on évite les doublons
    ...
    return {
        ...
        propositions: melanger(propositions)      // on mélange les 4 propositions
    };
}

// --- Générer un tableau de "nombre" questions ---
...
// --- N'afficher qu'un seul écran à la fois ---
function afficherEcran(ecran) {
    ...
    ecran.classList.remove("cachee");  // on retire "cachee" uniquement sur l'écran voulu
}
...
function repondre(texteChoisi) {
    ...
    if (texteChoisi === q.bonneReponse) {
        score = score + 1; // bonne réponse -> on incrémente le score
    }
...
function demarrerQuiz() {
    ...
    if (pseudo === "") {
        pseudo = "Joueur"; // pseudo par défaut si le champ est vide
    }
    score         = 0;
    indexQuestion = 0;
    questions     = genererQuestions(10); // on génère 10 questions
